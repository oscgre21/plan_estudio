...
// Load quiz data
console.log('📖 Loading science quiz data from V2...');
...
async function generateAllAudios() {
    console.log('\n🎵 Starting audio generation for Science Quiz V2...');
    ...
    for (let i = 0; i < quizData.length; i++) {
        ...
        if (item.question) {
            ...
            const questionFilename = `science-v2-q${i + 1}-${sanitizeFilename(questionText)}.mp3`;
            ...
            if (questionResult) {
                if (fs.existsSync(path.join(AUDIO_DIR, questionFilename)) &&
                    fs.statSync(path.join(AUDIO_DIR, questionFilename)).size > 0) {
                    successful++;
                } else {
                    skipped++;
                }
        ...
        if (item.explanation) {
            ...
            const explanationFilename = `science-v2-e${i + 1}-${sanitizeFilename(explanationText)}.mp3`;
            ...
            if (explanationResult) {
                if (fs.existsSync(path.join(AUDIO_DIR, explanationFilename)) &&
                    fs.statSync(path.join(AUDIO_DIR, explanationFilename)).size > 0) {
                    successful++;
                } else {
                    skipped++;
                }
...
generateAllAudios().then(() => {
    console.log('\n✨ Science Quiz V2 audio generation complete!');
    console.log('='.repeat(60));
}).catch(err => {
